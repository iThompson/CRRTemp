...
OI::OI() {
	...
	m_sStick = new Joystick(DRV_JOY_SINGLE);
...
double OI::GetTurnSingle(){
	if(m_sStick->GetTwist() > -.03 && m_sStick->GetTwist() < .03) return 0; // Add a deadzone to the joystick
		else return m_sStick->GetTwist();
}

double OI::GetYSingle(){
	if(m_sStick->GetY() > -.03 && m_sStick->GetY() < .03) return 0; // Add a deadzone to the joystick
		else return m_sStick->GetY();
}

double OI::GetXSingle(){
	if(m_sStick->GetX() > -.03 && m_sStick->GetX() < .03) return 0; // Add a deadzone to the joystick
		else return m_sStick->GetX();
}

double OI::GetXLeft() {
	if(m_lStick->GetX() > -.03 && m_lStick->GetX() < .03) return 0; // Add a deadzone to the joystick
	else return m_lStick->GetX();
}

double OI::GetXRight() {
	if(m_rStick->GetX() > -.03 && m_rStick->GetX() < .03) return 0; // Add a deadzone to the joystick
	else return m_rStick->GetX();
}
